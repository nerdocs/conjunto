...
# Credits:
# This is basically a modified copy of wagtail.models.audit_log.*
...
class LogEntryQuerySet(models.QuerySet):
    """A QuerySet that per default only displays objects on `.all()` that are public.

    It also provides a custom method for filtering by content type, to allow
    overriding on log entry models."""

    def public(self):
        """Returns only objects that are public."""
    ...
    def get_users(self):
        ...
        return UserModel.objects.filter(pk__in=self.get_user_ids()).order_by(
...
class BaseLogEntryManager(models.Manager):
    ...
    def log_action(
        self, instance: models.Model, action: str, **kwargs
    ) -> "BaseLogEntry":
        """
        Attributes:
            instance: The model instance we are logging an action for
            action: The action. Should be namespaced to app (e.g. wagtail.create, wagtail.workflow.start)
            kwargs: Addition fields to for the model deriving from BaseLogEntry
    ...
    def for_instance(self, instance) -> LogEntryQuerySet:
        """
...
class BaseLogEntry(models.Model):
    ...
    uuid = models.UUIDField(
        ...
        editable=False,
        help_text=_(
            "Log entries that happened as part of the same user action are "
            "assigned the same UUID"
        ),
    )
    ...
    public = models.BooleanField(
        default=True, help_text=_("This is a public log entry")
    )
    ...
    @cached_property
    def object_verbose_name(self) -> str:  # FIXME: |int ?
...
class ModelLogEntryManager(BaseLogEntryManager):
    ...
    def for_instance(self, instance: models.Model) -> QuerySet["ModelLogEntry"]:
        return self.filter(
            content_type=ContentType.objects.get_for_model(
                instance, for_concrete_model=False
            ),
